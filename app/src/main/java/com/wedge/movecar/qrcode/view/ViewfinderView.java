...
import com.wedge.movecar.qrcode.decoding.ResultPoint;
...
//        Rect frame = new Rect();
//        this.getLocalVisibleRect(frame);
        ...
        if (resultBitmap != null) {
            ...
            //车牌区域 区间控制在160-120的区间
            paint.setStyle(Paint.Style.STROKE);//设置填空 
            canvas.drawRect(frame.left + (frame.width() / 2) - 80, frame.bottom - 120, frame.left + (frame.width() / 2) + 80, frame.bottom - 70, paint);
            canvas.drawRect(frame.left + (frame.width() / 2) - 81, frame.bottom - 121, frame.left + (frame.width() / 2) + 81, frame.bottom - 69, paint);
