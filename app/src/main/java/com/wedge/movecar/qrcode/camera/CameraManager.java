 ...
 * 
 * 该对象将相机的服务对象和期望是唯一一个跟它说话。的实现封装需要预览步骤大小的图像，这是用于两个预览和解码。
 * 
 */
...
//	private static final int MAX_FRAME_HEIGHT = 360;
	 ...
	 * 告诉镜头硬件聚焦
	 * 
	 * handler 用于聚焦结束的唤醒 message 传递的消息
	 * 
	 * 
	 * @param handler
	...
	public RGBLuminanceSource buildRGBLuminanceSource(int[] data, int width,
			int height) {
		Rect rect = getFramingRectInPreview();
		// int previewFormat = configManager.getPreviewFormat();
		// String previewFormatString = configManager.getPreviewFormatString();

		return new RGBLuminanceSource(data, width, height, rect.left, rect.top,
